...
export namespace FlatContainerServices {
  ...
  export function verifyAtomIsSetqAndGlobalized(
    atoms: Array<ILispFragment>,
    from: ILispFragment | number
  ): boolean {
    const [index, atom] =
      typeof from === "number" ? [from, atoms[from]] : [from.flatIndex, from];
    const parentAtom = getParentAtomIfValidSetq(atoms, index);
    return !parentAtom
      ? false
      : hasGlobalizationContext(atom, parentAtom, atoms);
  }
  ...
  export function verifyAtomIsDefunAndGlobalized(
    atoms: Array<ILispFragment>,
    from: ILispFragment
  ): boolean {
  ...
  function getParentRootAtomIfOfType(
    atoms: Array<ILispFragment>,
    fromIndex: number,
    ...rootTypes: Array<string>
  ): ILispFragment | null {
  ...
  function lookBehindForParentRootAtom(
    atoms: Array<ILispFragment>,
    fromIndex: number
  ): ILispFragment | null {
    const initAtom = atoms[fromIndex];
    const parentParenAtom = lookBehindForParentParenthesisAtom(
      atoms,
      initAtom.flatIndex
    );
  ...
  function isValidSetqContext(
    atoms: Array<ILispFragment>,
    from: ILispFragment | number
  ): boolean {
    const previous =
      atoms[(typeof from === "number" ? from : from.flatIndex) - 1];
    return (
      previous instanceof LispAtom &&
      previous.symbol !== "'" &&
      (previous.isPrimitive() ||
        StringEqualsIgnoreCase(previous.symbol, "setq"))
    );
  }

  const GlobalFlag = "@GLOBAL";
  function hasGlobalizationContext(
    source: ILispFragment,
    parent: ILispFragment,
    atoms: Array<ILispFragment>
  ): boolean {
    ...
    return Boolean(
      aheadComment?.symbol.toUpperCase().includes(GlobalFlag) ||
  ...
  function lookBehindForBlockComment(
    ...
    const normalizedLineCount =
      previousAtom.symbol.split("\n").length + previousAtom.line;
    return previousAtom.isComment() &&
      normalizedLineCount === parentParenAtom.line
      ? previousAtom
      : null;
  }

  function lookBehindForParentParenthesisAtom(
    atoms: Array<ILispFragment>,
    fromIndex: number
  ): ILispFragment | null {
    let parenScope = 1;
  ...
  const commentFlag = ";";
  function lookAheadForInlineComment(
    atoms: Array<ILispFragment>,
    fromIndex: number
  ): ILispFragment | null {
