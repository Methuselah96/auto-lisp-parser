...
export class LispContainer extends LispAtom {
    ...
    if (this.contains(loc)) {
      this.atoms.forEach((lispObj) => {
        if (lispObj.contains(loc) && lispObj instanceof LispContainer) {
          result =
            lispObj.getExpressionFromPos(loc, parent) ??
            (parent ? this : lispObj);
        }
      });
    }
  ...
  getRange(): Range {
    const begin = this.getFirstAtom().getRange().getStartPosition();
    const close = this.getLastAtom().getRange().getEndPosition();
    return Range.fromPositions(begin, close);
  }
  ...
  private isValidForSetq(atom: ILispFragment): boolean {
    return (
      ...
      (atom instanceof LispContainer || atom.symbol.trim().length > 0)
    );
  }
