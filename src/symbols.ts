...
export namespace SymbolManager {
  ...
  export function updateOrCreateSymbolMap(
...
export interface ISymbolHost extends ISymbolBase {
  ...
  collectAllSymbols(
    allSymbols?: Map<string, Array<ISymbolReference>>
  ): Map<string, Array<ISymbolReference>>;
...
class AnonymousSymbolHost implements ISymbolHost {
  ...
  constructor(
    owner: ISymbolHost,
    fsPath: string,
    source: LispContainer,
    isProcessedBySubClass = false
  ) {
  ...
  equal(obj: ISymbolBase): boolean {
    if (obj instanceof AnonymousSymbolHost) {
      return (
        this.filePath === obj.filePath && this.range.equalsRange(obj.range)
      );
    }
    return false;
  }
  ...
  protected processLocalizationHeader(source: ILispFragment) {
    if (source instanceof LispContainer) {
      ...
      source.atoms.forEach((item) => {
        if (
          !item.isPrimitive() &&
          !SymbolServices.isNative(item.symbol.toLowerCase())
        ) {
          this.items.push(
            new NamedSymbolReference(this, this.filePath, item).withLocalFlag()
          );
        }
      });
    } else if (
      !source.isPrimitive() &&
      !SymbolServices.isNative(source.symbol.toLowerCase())
    ) {
      // Is foreach or vlax-for
      this.items.push(
        new NamedSymbolReference(this, this.filePath, source).withLocalFlag()
      );
    }
...
class NamedSymbolHost extends AnonymousSymbolHost {
  ...
  constructor(owner: ISymbolHost, fsPath: string, source: LispContainer) {
    ...
    this.named = new NamedSymbolReference(
      this,
      fsPath,
      source.getNthKeyAtom(1)
    ).withDefunFlag();
