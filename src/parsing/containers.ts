...
export class ContainerBuildContext {
  idx: number = 0;
  line: number = 0;
  column: number = 0;
  atomIndex: number = 0;
  ...
  createAtomFromTempAndReset(createIn: number, newState?: ParseState): void {
    this.createAtomInContainer(
      createIn,
      this.groupLine,
      this.groupColumn,
      this.temp.materialize()
    );
      ...
      this.basicSymbolMap.get(key)!.forEach((flatIndex) => {
        const flag =
          FlatContainerServices.verifyAtomIsDefunAndGlobalized(
            this.flatView,
            this.flatView[flatIndex]
          ) ||
          FlatContainerServices.verifyAtomIsSetqAndGlobalized(
            this.flatView,
            this.flatView[flatIndex]
          );
        if (flag) {
...
export function getDocumentContainer(
  ContextOrContent: ContainerBuildContext | string
): LispContainer {
  const ctx =
    typeof ContextOrContent === "string"
      ? new ContainerBuildContext(ContextOrContent)
      : ContextOrContent;
    ...
    if (ctx.state === ParseState.UNKNOWN) {
      if (curr === openParen || (curr === sglQuote && next === openParen)) {
        ...
        if (ctx.depth > 1 && ctx.containerIsEmpty(containerIndex)) {
          ...
          ctx.containerView[containerIndex].atoms.push(
            ctx.containerView[_getDocumentContainer(ctx)]
          );
          handled = true;
        }
